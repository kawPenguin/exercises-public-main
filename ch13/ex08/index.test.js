...
describe("fetchFirstFileSize", () => {
  it("ディレクトリにファイルが存在する場合、最初のファイルのサイズを返す", async () => {
    // readdir がファイルリストを返すように設定
    readdir.mockResolvedValue(["file1.txt", "file2.txt"]);
    // stat が特定のファイルサイズ情報を持つオブジェクトを返すように設定
    stat.mockResolvedValue({ size: 123 });

    // テストを実行し、結果が 123 であることを期待
    await expect(fetchFirstFileSize("/fake/path")).resolves.toBe(123);
  });

  it("ディレクトリが空の場合、null を返す", async () => {
    // readdir が空の配列を返すように設定
    readdir.mockResolvedValue([]);

    await expect(fetchFirstFileSize("/fake/empty")).resolves.toBeNull();
  });
});
